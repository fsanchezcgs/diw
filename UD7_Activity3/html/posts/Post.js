export default {
  name: "Post",
  props: ["title", "content", "author", "creationDate", "image"],
  methods: {
  ...
  <div class="bg-primary p-2 rounded-2 my-2">
    <h4>{{title}}</h4>
    <p class="content rounded-2 p-2">{{content}}</p>
    ...
    <div>
      ...
      <input type="button" value="Delete" @click="deletePost(index)" class="mx-1 p-1 rounded-1">
    </div>
  </div>`,
};
